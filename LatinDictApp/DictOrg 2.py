...
def DictOrg(text):
    ...
    for chunk in chunks:
        ...
        if not chunk or chunk == []:
            ...
            filteredSources = re.findall(r'\b[a-zA-Z]+(?:[a-zA-Z]+)?\b', str(sources))
            ...
            print("filtered sources:", filteredSources)
            ...
            for i in range(len(Def)):
                ...
                for lat_word ,sourc_word in zip(Latin_list,sources):
                        # if i < len(Latin_list):
                    if len(Latin_list) == len(filteredSources):
                        ...
                        Sources_word.text = str(filteredSources) 
                else:
                    Latin_word = ET.SubElement(English_words,"Latin")
...
#for line in content:
textex = '''// /general city: King's Mountain, Königsberg, Monterrey, Montréal  ► Regi(o)montium, i n.
...
// Atlantic islands: Azores  ►► Tertiariae Insulae (1652 TURS. 263)// Atlantic islands: Canary Islands  ► Insulae Fortûnâtae (f. pl.)  ¶ PLIN.  1595 MERCATOR II "Europa."  1794 RUIZ viii.  ► Insulae Canariae (f. pl.)  ¶ Arn.  1595 MERCATOR II"Europa."// Atlantic islands: Canary Islands: Tenerife  ► Teneriffa, ae f.  ¶ 1794 RUIZ viii.  ► Ninguâria, ae f.  ¶ Plin. 6, 204.  Egger N.L.  ► Nivâria, ae f.  ¶ Plin. 6, 204 (variant reading). Egger N.L.'''
